...
No conventional page layout: the canvas fills the viewport as before. The HUD is now a fixed instrument cluster rather than scattered corner chips: a Level dial + pause toggle grouped top-left, a Plasma dial + Tech Tree button grouped top-right (the button sits *below* the dial's full footprint, not beside it, to clear its caption label), and a thin XP tape spanning the full top edge beneath both. Modals are centered chamfered console panels (`min(92vw, 420–560px)` depending on content) over a full-viewport scrim — a deliberate departure from the old system's full-bleed, flush-top sheets.
